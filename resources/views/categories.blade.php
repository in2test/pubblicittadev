<x-layout>
    ...
    <section class="mb-12 mt-24 px-8 3xl:px-32">
                    ...
                    <nav
                        ...
                        @if ($cat->parent)
                            <a class="hover:text-primary transition-colors"
                                href="{{ route('category', $cat->parent->slug) }}">{{ $cat->parent->name }}</a>
                            <span class="material-symbols-outlined text-[10px]">chevron_right</span>
                        @endif

                        <span class="text-on-surface font-bold">{{ $cat->name }}</span>
                    </nav>
                @endif
            </div>
            <div class="text-right">
                <span class="text-3xl font-light tracking-tighter text-on-surface">{{ $products->total() }}</span>
        ...
        <div class="bg-surface-container-lowest p-1 shadow-sm flex items-center gap-2 border-b-2 border-primary">
            <form action="{{ route('catalog') }}" method="GET" class="flex flex-1 items-center gap-2">
                <span class="material-symbols-outlined px-3 text-secondary">search</span>
                <input class="flex-1 border-none focus:ring-0 text-sm bg-transparent py-4 font-body" name="search"
                    value="{{ $search ?? '' }}" placeholder="Filtra per nome prodotto, SKU o specifica tecnica..."
                    type="text" />
                <button type="submit"
                ...
                <nav class="flex items-center justify-center gap-2">
                    ...
                    @foreach ($products->getUrlRange(1, $products->lastPage()) as $page => $url)
                        @if ($page == $products->currentPage())
                            <span
                                class="w-10 h-10 bg-primary text-white font-bold text-xs flex items-center justify-center">{{ str_pad($page, 2, '0', STR_PAD_LEFT) }}</span>
                        @else
                            <a href="{{ $url }}"
                                class="w-10 h-10 border border-slate-200 font-bold text-xs hover:bg-slate-50 flex items-center justify-center">{{ str_pad($page, 2, '0', STR_PAD_LEFT) }}</a>
                        @endif
                    @endforeach

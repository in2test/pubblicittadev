...
@php
    $availableSizes = $product->variations->pluck('size')->unique('id')->filter()->sortBy('sort_order');
@endphp
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
            @foreach ($availableSizes as $size)
                <div x-show="!activeColorId || colorToSizes[activeColorId]?.includes({{ $size->id }})"
                    :class="{ 'opacity-50 grayscale-[0.5]': activeColorId && colorSizeIsAvailable?.[activeColorId] && (!
                            colorSizeIsAvailable[activeColorId]?.[{{ $size->id }}] || colorSizeAvailability?.[
                                activeColorId
                            ]?.[{{ $size->id }}] === 0) }"
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-[9px] text-gray-500 uppercase tracking-tighter mt-1"
                            ...
                            <template x-if="colorSizeAvailability?.[activeColorId]?.[{{ $size->id }}] > 0">
                                <span>Disponibili: <span class="font-bold text-primary/70"
                                        x-text="colorSizeAvailability[activeColorId]?.[{{ $size->id }}]"></span></span>
                            </template>
                            <template x-if="colorSizeAvailability?.[activeColorId]?.[{{ $size->id }}] === 0">
                    ...
                    <div>
                        <input name="quantities[{{ $size->id }}]" type="number" min="0"
                            :max="activeColorId && colorSizeAvailability?.[activeColorId] ? colorSizeAvailability[
                                activeColorId][{{ $size->id }}] : null"
                            x-model.number="quantities[{{ $size->id }}]"
                            :disabled="activeColorId && colorSizeIsAvailable?.[activeColorId] && (!colorSizeIsAvailable[
                                    activeColorId][{{ $size->id }}] || colorSizeAvailability?.[activeColorId]
                                ?.[{{ $size->id }}] === 0)"
                            :placeholder="activeColorId && colorSizeAvailability?.[activeColorId] && colorSizeAvailability[
                                activeColorId]?.[{{ $size->id }}] === 0 ? 'X' : '0'"
